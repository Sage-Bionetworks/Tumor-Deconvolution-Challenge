...
coarse_submissions <- 
    "select id, dockerrepositoryname from syn22365821 where status = 'ACCEPTED'" %>% 
    query_synapse_table() %>% 
    dplyr::select(id, repo_name = dockerrepositoryname)

fine_submissions <- 
    "select id, dockerrepositoryname from syn22365822 where status = 'ACCEPTED'" %>% 
    query_synapse_table() %>% 
    dplyr::select(id, repo_name = dockerrepositoryname)
...
fine_gold_standard <- synapse_file_to_tbl("syn21752551",  delim = ",")
...
coarse_gold_standard <- synapse_file_to_tbl("syn21752552",  delim = ",")
