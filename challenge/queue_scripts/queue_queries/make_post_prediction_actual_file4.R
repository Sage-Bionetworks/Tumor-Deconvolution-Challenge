...
coarse_submissions <- 
    "select id, dockerrepositoryname from syn23517749 where status = 'ACCEPTED'" %>% 
    query_synapse_table() %>% 
    dplyr::select(id, repo_name = dockerrepositoryname)

fine_submissions <- 
    "select id, dockerrepositoryname from syn23517766 where status = 'ACCEPTED'" %>% 
    query_synapse_table() %>% 
    dplyr::select(id, repo_name = dockerrepositoryname)
...
fine_gold_standard <- synapse_file_to_tbl("syn23019512",  delim = ",")
...
coarse_gold_standard <- synapse_file_to_tbl("syn23019511",  delim = ",")
