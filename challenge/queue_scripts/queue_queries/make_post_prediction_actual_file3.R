...
coarse_submissions <- 
    "select id, dockerrepositoryname from syn22396060 where status = 'ACCEPTED'" %>% 
    query_synapse_table() %>% 
    dplyr::select(id, repo_name = dockerrepositoryname)

fine_submissions <- 
    "select id, dockerrepositoryname from syn22396079 where status = 'ACCEPTED'" %>% 
    query_synapse_table() %>% 
    dplyr::select(id, repo_name = dockerrepositoryname)
...
fine_gold_standard <- synapse_file_to_tbl("syn22392154",  delim = ",")
...
coarse_gold_standard <- synapse_file_to_tbl("syn22392153",  delim = ",")
