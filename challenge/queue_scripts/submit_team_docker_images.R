...
submitted_fine_images <-
    query_synapse_table("SELECT entityid FROM syn22276620") %>% 
    dplyr::pull(entityid)

all_images %>% 
    dplyr::filter(.data$submission_id %in% coarse_images) %>% 
    dplyr::pull(id) %>% 
    purrr::walk(make_submission, 9614582L)

all_images %>% 
    dplyr::filter(.data$submission_id %in% fine_images) %>%
    dplyr::filter(!.data$id %in% submitted_fine_images) %>%
    dplyr::pull(id) %>% 
