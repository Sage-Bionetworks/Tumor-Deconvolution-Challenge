...
synLogin()
##synId <- "syn21715094"
validation.results.file <- "insilico-results/all-predictions-cpm.tsv"
csx.results.file <- "insilico-results/validation-csx-all-gene-predictions-cpm.tsv"

prefix <- "is-tpm-"
title.postfix <- "; All Genes (TPM)"

validation.results.file <- "insilico-results/all-predictions-norm-cnts.tsv"
csx.results.file <- "insilico-results/validation-csx-all-gene-predictions-cnts-to-cpm.tsv"

prefix <- "is-norm-cnt-"
title.postfix <- "; All Genes (Norm Cnt)"
...
## path <- "../validation_data/qc/in-silico-validation-coarse.csv"
path <- "../validation_data/qc/in-silico-validation-coarse.csv"
path <- "../validation_data/qc/in-silico-gt-coarse.csv"
...
val.coarse <-
    ddply(val.coarse,
          ...
          .fun = function(df) {
              data.frame(measured = sum(df$measured))
          })
} # FALSE                         

##synId <- "syn21590365"
##path <- synGet(synId, downloadFile = TRUE)$path
path <- "../validation_data/qc/in-silico-validation-fine.csv"
path <- "../validation_data/qc/in-silico-gt-fine.csv"
val.fine <- read.table(path, sep = ",", header = TRUE, stringsAsFactors = FALSE)
val.fine$subchallenge <- "fine"

val.all <- rbind(val.coarse, val.fine[, colnames(val.coarse)])
...
res <- merge(res[, !(colnames(res) == "measured")], val.all,
             by.x = c("dataset.name", "sample.id", "cell.type", "subchallenge"),
             by.y = c("dataset.name", "sample.id", "cell.type", "subchallenge"))

res$sample.id <- paste0(res$dataset.name, res$sample.id)

source("../utils.R")

res$mixture.type <- "Biological"
flag <- grepl(res$sample.id, pattern="ISCR") | grepl(res$sample.id, pattern="ISFR")
res[flag, "mixture.type"] <- "Random"
