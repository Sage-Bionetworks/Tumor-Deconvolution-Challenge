...
# -> Determine frequency of sub-Challenge annotations with each patient, relative to only those cell types with challenge labels + cancer and to all cells (though we don’t report these results, as they are not consistent with how we defined the challenge admixtures). 
# -> create.pseudobulk by summing counts of cells with same sub-Challenge label
...
# -> Drop genes not used in the Challenge
# -> Drop the patients whose respective cells don't cluster with other the corresponding cell type from other patients. find.inconsistent.umap.cell.types: specifically, following a umap projection, if the most frequent annotation of a cell’s 10 nearest neighbors differs from it’s own annotation, it is considered problematic. This includes:
# CID4513_B.cells
# CID4461_monocytic.lineage
# CID4523_endothelial.cells
# CID4465_B.cells
# So the respective patients are excluded
#patients.to.exclude <- c("CID4513","CID4461","CID4523","CID4465") 
# The UMAP is created based only on CPMs of “deconvolution” genes used in several published methods: CIBERSORT’s LM22, EPIC’s “TRef” and “BRef” signature genes, quantiseq’s TIL10 genes, and MCP-Counter.
# -> translate.matrix from symbol to ensg using the same ensg to hugo symbol translation used in the challenge (syn22394938), where multiple ensg ids mapping to the same symbol are collapsed by summing them -> raw.ensg.mats
# -> normalize counts to max count
# -> norm.{ensg,sym}.raw.cnts
# -> create.in.silico.admixtures.from.pseudo.bulk using cell.type.freqs
...
```{r}
wu.metadata.dir <- '/projects/compsci/jgeorge/whitebr/wu-breast/'
wu.norm.expr.dir <- '/projects/compsci/jgeorge/whitebr/wu-breast/'
wu.raw.expr.dir <- '/projects/compsci/jgeorge/whitebr/wu-breast//BrCa_Atlas_Count_out/'
  ...
  for(cell.type.subset in subsets) {
    ...
    expr.mat.files[[grain]][[cell.type.subset]] <- 
      list("hugo-cpm" = paste0("wu-", grain, "-hugo-cpm-expr-", cell.type.subset, ".csv"),
           ...
           "ensg-cnts" = paste0("wu-", grain, "-ensg-raw-expr-", cell.type.subset, ".csv")
      )
