...
rownames(wu.coarse.metadata) <- wu.coarse.metadata$NAME
wu.fine.metadata <- subset(wu.metadata, !is.na(celltype_fine))[, c("NAME", "Patient", "celltype_fine")]
rownames(wu.fine.metadata) <- wu.fine.metadata$NAME
```
...
flag <- colnames(coarse.pb) %in% names(gene.lens)
table(flag)        
...
genes.with.lens <- intersect(names(gene.lens), colnames(coarse.pb))
if(FALSE) {
gene.lens <- gene.lens[genes.with.lens]
coarse.pb <- coarse.pb[, genes.with.lens]
fine.pb <- fine.pb[, genes.with.lens]
}
```

```{r}
coarse.raw.expr <- t(coarse.pb)
fine.raw.expr <- t(fine.pb)
```

```{r}
# Translate raw counts to TPMs
coarse.tpm.expr <- tpm(as.matrix(coarse.raw.expr)[genes.with.lens, ], gene.lens[genes.with.lens])
coarse.my.tpm.expr <- my.tpm(as.matrix(coarse.raw.expr)[genes.with.lens, ], gene.lens[genes.with.lens])
stopifnot(max(abs(coarse.tpm.expr-coarse.my.tpm.expr)) < 1e-5)

coarse.cpm.expr <- cpm(coarse.raw.expr)
coarse.my.cpm.expr <- my.cpm(coarse.raw.expr)
stopifnot(max(abs(coarse.cpm.expr-coarse.my.cpm.expr)) < 1e-5)

fine.tpm.expr <- tpm(as.matrix(fine.raw.expr)[genes.with.lens,], gene.lens[genes.with.lens])
fine.my.tpm.expr <- my.tpm(as.matrix(fine.raw.expr)[genes.with.lens,], gene.lens[genes.with.lens])
stopifnot(max(abs(fine.tpm.expr-fine.my.tpm.expr)) < 1e-5)

fine.cpm.expr <- cpm(fine.raw.expr)
fine.my.cpm.expr <- my.cpm(fine.raw.expr)
stopifnot(max(abs(fine.cpm.expr-fine.my.cpm.expr)) < 1e-5)
...
missing.genes <- challenge.genes[!(challenge.genes %in% rownames(coarse.raw.expr))]

print(table(rownames(coarse.raw.expr) %in% challenge.genes))
#FALSE  TRUE 
#11739 17994 
print(table(challenge.genes %in% rownames(coarse.raw.expr)))
#FALSE  TRUE 
#10517 17994
add.zeroes <- FALSE
restrict.to.common.genes <- FALSE
if(add.zeroes) {
  zero.mat <- matrix(data = 0, ncol = ncol(coarse.raw.expr), nrow = length(missing.genes))
  rownames(zero.mat) <- missing.genes
  colnames(zero.mat) <- colnames(coarse.raw.expr)
  coarse.raw.expr <- rbind(coarse.raw.expr, zero.mat)
  coarse.cpm.expr <- rbind(coarse.cpm.expr, zero.mat)
  coarse.tpm.expr <- rbind(coarse.tpm.expr, zero.mat)
}

common.genes <- intersect(rownames(coarse.raw.expr), challenge.genes)

if(restrict.to.common.genes) {
  coarse.raw.expr <- coarse.raw.expr[common.genes,]
  coarse.cpm.expr <- coarse.cpm.expr[common.genes,]
  coarse.tpm.expr <- coarse.tpm.expr[common.genes,]
}

if(add.zeroes) {
  zero.mat <- matrix(data = 0, ncol = ncol(fine.raw.expr), nrow = length(missing.genes))
  rownames(zero.mat) <- missing.genes
  colnames(zero.mat) <- colnames(fine.raw.expr)
  fine.raw.expr <- rbind(fine.raw.expr, zero.mat)
  fine.cpm.expr <- rbind(fine.cpm.expr, zero.mat)
  fine.tpm.expr <- rbind(fine.tpm.expr, zero.mat)
}

common.genes <- intersect(rownames(fine.raw.expr), challenge.genes)
if(restrict.to.common.genes) {
  fine.raw.expr <- fine.raw.expr[common.genes,]
  fine.cpm.expr <- fine.cpm.expr[common.genes,]
  fine.tpm.expr <- fine.tpm.expr[common.genes,]
}
```


```{r}
res <- MCPcounter.estimate(coarse.tpm.expr, featuresType="HUGO_symbols")
res <- melt(res)
...
umap <- umap(t(coarse.cpm.expr[rownames(coarse.cpm.expr) %in% deconv.genes,]))
df <- data.frame(umap1 = umap$layout[,1], umap2 = umap$layout[,2], sample = rownames(umap$layout))
splitf <- sapply(stringr::str_split(df$sample, 
                                    pattern = "_",n = 2), `[`, 2)
df$cell.type <- splitf
cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
ggplot(data = df, aes(x = umap1, y = umap2, colour = cell.type)) + geom_point() + scale_colour_manual(values = cbbPalette)
ggplot(data = df, aes(x = umap1, y = umap2, colour = cell.type)) + geom_point() + scale_colour_manual(values = cbbPalette) + geom_text(aes(label = cell.type))
# problematic:
# CID4513_B.cells
...
coarse.cell.type.freq <- subset(coarse.cell.type.freq, !(Patient %in% patients.to.exclude))
fine.cell.type.freq <- subset(fine.cell.type.freq, !(Patient %in% patients.to.exclude))
exclude.pattern <- paste0(patients.to.exclude, collapse="|")
coarse.exclude.flag <- grepl(pattern=exclude.pattern, colnames(coarse.raw.expr))
fine.exclude.flag <- grepl(pattern=exclude.pattern, colnames(fine.raw.expr))

```

```{r}
# Plot coarse after excluding
umap <- umap(t(coarse.cpm.expr[rownames(coarse.cpm.expr) %in% deconv.genes,!coarse.exclude.flag]))
df <- data.frame(umap1 = umap$layout[,1], umap2 = umap$layout[,2], sample = rownames(umap$layout))
splitf <- sapply(stringr::str_split(df$sample, 
                                    pattern = "_",n = 2), `[`, 2)
df$cell.type <- splitf
...
# Repeat with TPM
umap <- umap(as.matrix(t(coarse.tpm.expr[rownames(coarse.tpm.expr) %in% deconv.genes,!coarse.exclude.flag])))
df <- data.frame(umap1 = umap$layout[,1], umap2 = umap$layout[,2], sample = rownames(umap$layout))
splitf <- sapply(stringr::str_split(df$sample, 
                                    pattern = "_",n = 2), `[`, 2)
df$cell.type <- splitf
...
# umap <- umap(t(fine.cpm.expr[rownames(fine.cpm.expr) %in% deconv.genes,!fine.exclude.flag]))
umap <- umap(t(fine.cpm.expr[rownames(fine.cpm.expr) %in% deconv.genes,]))
df <- data.frame(umap1 = umap$layout[,1], umap2 = umap$layout[,2], sample = rownames(umap$layout))
splitf <- sapply(stringr::str_split(df$sample, 
                                    pattern = "_",n = 2), `[`, 2)
df$cell.type <- splitf
#p_load(colorBlindness)
df <- df[order(df$umap1, df$umap2),]
cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
ggplot(data = df, aes(x = umap1, y = umap2, colour = cell.type)) + geom_point() + scale_colour_manual(values = Blue2OrangeRed14Steps)
ggplot(data = df, aes(x = umap1, y = umap2, colour = cell.type)) + geom_point() + scale_colour_manual(values = Blue2OrangeRed14Steps) + geom_text(aes(label = cell.type))
...
flag <- df$cell.type %in% c("BRCA", "endothelial.cells", "fibroblasts", "macrophages", "myeloid.dendritic.cells", "monocytes")
tmp <- df[!flag,c(1,2)]
...
```{r}
# Drop the 4 patients whose respective cells don't cluster with other the corresponding cell type from other patients
coarse.raw.expr <- coarse.raw.expr[, !coarse.exclude.flag]
coarse.cpm.expr <- coarse.cpm.expr[, !coarse.exclude.flag]
coarse.tpm.expr <- coarse.tpm.expr[, !coarse.exclude.flag]

fine.raw.expr <- fine.raw.expr[, !fine.exclude.flag]
fine.cpm.expr <- fine.cpm.expr[, !fine.exclude.flag]
fine.tpm.expr <- fine.tpm.expr[, !fine.exclude.flag]

```

```{r}
# Normalize the count matrix so that each sample has the same number of counts (set to the max of all samples)
tot.cnts <- colSums(coarse.raw.expr)
max.tot.cnts <- max(tot.cnts)
coarse.norm.raw.expr <- sweep(coarse.raw.expr, 2, colSums(coarse.raw.expr),`/`) * max.tot.cnts

tot.cnts <- colSums(fine.raw.expr)
max.tot.cnts <- max(tot.cnts)
fine.norm.raw.expr <- sweep(fine.raw.expr, 2, colSums(fine.raw.expr),`/`) * max.tot.cnts
...
```{r}
# Combine into pseudobulk for each patient based on observed frequencies ...

# ... for both TPMs ...
coarse.tpm.admixture <- create.in.silico.admixtures.from.pseudo.bulk(coarse.tpm.expr, coarse.cell.type.freq, sample.col = "Patient", cell.type.col = "celltype_coarse", fraction.col = "frac")
coarse.tpm.admixture <- cbind(Gene = rownames(coarse.tpm.admixture), as.data.frame(coarse.tpm.admixture))

# ... and raw cnts ...
coarse.raw.admixture <- create.in.silico.admixtures.from.pseudo.bulk(coarse.norm.raw.expr, coarse.cell.type.freq, sample.col = "Patient", cell.type.col = "celltype_coarse", fraction.col = "frac")
coarse.raw.admixture <- cbind(Gene = rownames(coarse.raw.admixture), as.data.frame(coarse.raw.admixture))

fine.tpm.admixture <- create.in.silico.admixtures.from.pseudo.bulk(fine.tpm.expr, fine.cell.type.freq, sample.col = "Patient", cell.type.col = "celltype_fine", fraction.col = "frac")
fine.tpm.admixture <- cbind(Gene = rownames(fine.tpm.admixture), as.data.frame(fine.tpm.admixture))

# ... and raw cnts ...
fine.raw.admixture <- create.in.silico.admixtures.from.pseudo.bulk(fine.norm.raw.expr, fine.cell.type.freq, sample.col = "Patient", cell.type.col = "celltype_fine", fraction.col = "frac")
fine.raw.admixture <- cbind(Gene = rownames(fine.raw.admixture), as.data.frame(fine.raw.admixture))
...
ensg.to.hugo.trans.tbl <- ensg.to.hugo.trans.tbl[, c("hugo", "ensg")]
...
run.da505.fine <- function(mat) {
  run.da505_(mat, grain="finegrain")
}

run.da505.coarse <- function(mat) {
  run.da505_(mat, grain="coarsegrain")
}
...
```{r}
plot.deconv.results <- function(res) {
  ...
  g <- ggscatter(res, x = "frac", y = "value",
...
```{r}
run.cibersort_ <- function(tpm.expr, translation_df, file) {
  write.table(file = file, tpm.expr, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
  res <- CIBERSORT('LM22.txt',file, QN = FALSE, absmean = TRUE, abs_method = 'sig.score')
  res <- melt(res)
...
run.cibersort.fine <- function(tpm.expr, file) {

  # Translate cibersort output to challenge subtypes
  
  run.cibersort_(tpm.expr, cibersort_fine_translation_df, file)
...
run.cibersort.coarse <- function(tpm.expr, file) {

  # Translate cibersort output to challenge subtypes
  
  run.cibersort_(tpm.expr, cibersort_coarse_translation_df, file)
...
fine.gs <- subset(fine.gs, cell.type != "BRCA")
...
coarse.gs <- subset(coarse.gs, cell.type != "BRCA")
