...
expr.mat  <- expr_id %>% 
    create_df_from_synapse_id() %>% 
    dplyr::rename(Gene = `_TranscriptID`) %>% 
    tidyr::pivot_longer(-Gene) %>% 
    dplyr::arrange(name) %>% 
    dplyr::inner_join(translation_df, by = c("name" = "cell_type")) %>% 
    dplyr::select(-name) 
...
ensg.to.symbol.map <- get.symbol.to.ensg.map(expr.mat$Gene)
symbol.to.symbol.map <- dplyr::mutate(ensg.to.symbol.map, to = from)

compression.fun <- "colMeans"
symbol.compression.fun <- compression.fun
ensg.compression.fun <- compression.fun
expr.mat.symbol <- translate.genes(expr.mat, symbol.to.symbol.map, fun = symbol.compression.fun)
expr.mat.ensg <- translate.genes(expr.mat, ensg.to.symbol.map, fun = ensg.compression.fun)
...
coarse_mix_tbl2 <- coarse_mix_tbl %>% 
    dplyr::select(mix, freqs, samples, cell.types) %>% 
