...
## foo <- sample.hierarchical.model(n.admixtures, azizi.hierarchical.model, pops = final.pops, min.prop = min.prop)

## bar <- sample.hierarchical.model(n.admixtures, cibersort.hierarchical.model, pops = final.pops, min.prop = min.prop)
...
flat.model["min",] <- unlist(lapply(flat.model["min",], function(x) max(x, min.prop)))
flat.model["max",] <- unlist(lapply(flat.model["max",], function(x) max(x, min.prop)))
...
## Set cancer range from 0.2 to 0.8
flat.model["max",tumor.col] <- 0.8
flat.model["min",tumor.col] <- 0.2

populations <- colnames(flat.model)
## Restrict non-tumor population to be between min.prop and 0.5
lbs <- rep(min.prop, length(populations))
...
mats <-
    llply(1:length(tumor.mins), .parallel = FALSE,
          .fun = function(i) {
              lbs[which.tumor.col] <- min.prop
              ubs[which.tumor.col] <- tumor.maxs[i] - tumor.mins[i] - min.prop
              mat <- generate.random.uniform.admixtures(populations, 100, min.prop = min.prop, max.prop = 1 - tumor.mins[i],
	                                                lbs = lbs, ubs = ubs, step = step)
...
mat <- generate.random.uniform.admixtures(populations, 1000, min.prop = min.prop, max.prop = 1, lbs = lbs, ubs = ubs, step = step)
mat <- mat[!duplicated(mat),]
  ...
  llply(nms,
        .fun = function(target.pop) {
		 ...
		 other.pops <- populations[!(populations %in% c(src.pop, target.pop))]
	         ...
	         lbs <- rep(min.prop, length(tmp.pops))
		 ...
		 ubs[src.indx] <- max.pair.pop.for.zeroing		 
                 ...
                 tmp <- llply(1:length(tumor.mins), .parallel = FALSE,
		              .fun = function(i) {
			               tmp.lbs <- lbs
				       tmp.ubs <- ubs
				       tmp.lbs[which(other.pops == tumor.col)] <- min.prop
				       tmp.ubs[which(other.pops == tumor.col)] <- tumor.maxs[i] - tumor.mins[i] - min.prop
                                       mat <- generate.random.uniform.admixtures(tmp.pops, 1000, min.prop = min.prop,
				                                                 max.prop = 1 - tumor.mins[i],
		                                                                 lbs = tmp.lbs, ubs = tmp.ubs, step = step)
...
mats <-
  llply(nms,
        .fun = function(target.pop) {
		 ...
		 lbs[src.indx] <- min.pair.pop.for.zeroing
		 ubs[src.indx] <- max.pair.pop.for.zeroing		 
                 mat <- generate.random.uniform.admixtures(tmp.pops, 1000, min.prop = min.prop, max.prop = 1,
		                                           lbs = lbs, ubs = ubs, step = step)
...
##load("flat.model.Rd")
tumor.mins <- seq(from = 0.2, to = 0.8, by = min.prop)
...
tumor.flat.model["min", "tumor.fraction"] <- min(tumor.mins)
tumor.flat.model["max", "tumor.fraction"] <- max(tumor.mins)
  ...
  llply(1:length(tumor.mins), .parallel = TRUE,
        .fun = function(i) {
		 ...
		 model <- model[, !(colnames(model) == "tumor.fraction")]
			     ...
			     if(step > 0.01) { stop("Was expecting step less than 1%\n") }
			     ## The following 3 lines ensure we have a multiple of step size
                             df <- df / ( step * 100 )
