...
zero.out.populations <- function(tbl) {

  cols.to.zero.out <- colnames(tbl)
  ...
  j.indices <- unlist(lapply(cols.to.zero.out, function(col) which(colnames(tlb) == col)[1]))

  ## randomly zero out columns in the broken admixtures
  min.to.zero.out <- 0.02
  indices.to.zero.out <- 1:nrow(tbl)
  for(j in j.indices) {
    zeroedout <- FALSE
    for(i in 1:indices.to.zero.out) {
      
      if(tbl[i, j] > min.to.zero.out) {
        val < tbl[i,j]
	indx <- j+1
	if(indx > ncol(tbl)) { indx <- j-1 }
	tbl[i,indx] <- tbl[i,indx] + val
	tbl[i,j] <- 0
	zeroedout <- TRUE
	break
      }
    }
    if(!zeroedout) { stop(paste0("Could not find ", colnames(tbl)[j], " to zero out\n")) }
  }
  tbl
}
...
addWorksheet(wb, "random-unconstrained-admixtures-zero")
writeData(wb, sheet = 5, broken.stick.admixtures.unconstrained.zero, colNames = TRUE, rowNames = FALSE)

addWorksheet(wb, "random-admixtures-zero")
writeData(wb, sheet = 6, broken.stick.admixtures.zero, colNames = TRUE, rowNames = FALSE)
