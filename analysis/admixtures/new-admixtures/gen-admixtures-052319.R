...
if(use.eosinophils) {
  translation[["granulocytes"]] <- c("granulocytes")
} else {
  translation[["Neutrophils"]] <- c("granulocytes")
}
...
gran.cols <- c("Neutrophils", "Eosinophils")
azizi.granulocyte.frac.df <-
    azizi.cnt.df[, gran.cols] / rowSums(azizi.cnt.df[, gran.cols])
...
azizi.granulocyte.unif.orig.fits <- calc.col.min.max(azizi.granulocyte.frac.df, min.prob = min.prob, max.prob = max.prob)
colnames(azizi.granulocyte.unif.orig.fits) <- c("population", "min", "max")
...
azizi.granulocyte.unif.fits <- clean.up.min.max.params(azizi.granulocyte.unif.orig.fits)
...
azizi.granulocyte.unif.fits <- invert.min.max.params(azizi.granulocyte.unif.fits)
...
## - Lambrechts -> tumor, Fibroblasts, Endothelial, NK, Tregs, mono.macro, DC, granulocytes, CD4, CD8, B
## if we are using eosinophils
## - granulocytes -> Azizi -> Neutrophils, Eosinophils
## if we are not using eosinohils
...
if(use.eosinophils) {
  lambrechts.hierarchical.model <-
    list(
          list(parent = "root",
               children = c("tumor.fraction", "Fibroblasts", "Endothelial_cells", "Dendritic_cells",
                            "NK_cells", "Tregs", "granulocytes", "CD4_T_cells", "CD8_T_cells", "B_cells", "mono.macro"),
               dist = "unif", params = lambrechts.unif.fits),
      	  list(parent = "CD4_T_cells",
	       children = c("Memory_CD4_T_cells", "Naive_CD4_T_cells"),
	       dist = "unif", params = cytof.cd4.t.cell.unif.fits),
	  ...
	  list(parent = "mono.macro",
	       children = c("Monocytes", "Macrophages"),
	       dist = "unif", params = azizi.mono.macro.unif.fits),
	  list(parent = "granulocytes",
	       children = c("Neutrophils", "Eosinophils"),
	       dist = "unif", params = azizi.granulocyte.unif.fits))
}
...
mats <-
    llply(1:length(tumor.mins), .parallel = FALSE,
          .fun = function(i) {
              ...
              mat <- generate.random.uniform.admixtures(populations, 100, min.prop = 0.01, max.prop = 1 - tumor.mins[i],
	                                                lbs = lbs, ubs = ubs)
...
broken.stick.admixtures.all <- mat
broken.stick.admixtures <- t(find.extremal.points(t(mat), m = 200))

set.seed(1234)
lbs <- rep(0.01, length(populations))
ubs <- rep(0.5, length(populations))
ubs[which.tumor.col] <- 1
mat <- generate.random.uniform.admixtures(populations, 1000, min.prop = 0.01, max.prop = 1, lbs = lbs, ubs = ubs)
mat <- mat[!duplicated(mat),]
broken.stick.admixtures.unconstrained.all <- t(mat)

broken.stick.admixtures.unconstrained <- t(find.extremal.points(mat, m = 200))
...
min.pair.pop.for.zeroing <- 0.2
max.pair.pop.for.zeroing <- 0.35

set.seed(1234)
nms <- names(pops.to.zero.out)
names(nms) <- nms
mats <-
  llply(nms,
        .fun = function(target.pop) {
	         src.pop <- pops.to.zero.out[[target.pop]]
		 other.pops <- populations[!(populations %in% c(src.pop, target.pop))]
                 tmp.pops <- c(src.pop, other.pops)		 
	         lbs <- rep(0.01, length(tmp.pops))
                 ubs <- rep(0.5, length(tmp.pops))
		 src.indx <- 1
		 lbs[src.indx] <- min.pair.pop.for.zeroing
		 ubs[src.indx] <- max.pair.pop.for.zeroing		 

                 tmp <- llply(1:length(tumor.mins), .parallel = FALSE,
		              .fun = function(i) {
			               tmp.lbs <- lbs
				       tmp.ubs <- ubs
				       tmp.lbs[which(other.pops == tumor.col)] <- 0.01
				       tmp.ubs[which(other.pops == tumor.col)] <- tumor.maxs[i] - tumor.mins[i] - 0.01
                                       mat <- generate.random.uniform.admixtures(tmp.pops, 1000, min.prop = 0.01,
				                                                 max.prop = 1 - tumor.mins[i],
		                                                                 lbs = tmp.lbs, ubs = tmp.ubs)
                                       mat[which(rownames(mat)==tumor.col),] <- mat[tumor.col,] + tumor.mins[i]
				       mat
				     })
		 mat <- Reduce("cbind", tmp)
		 vec <- rep(0, ncol(mat))
		 colnames(mat) <- NULL
		 mat <- rbind(mat, vec)
		 rownames(mat)[nrow(mat)] <- target.pop
		 mat <- mat[populations, ]
		 mat
	})
...
broken.stick.admixtures.zero <- t(find.extremal.points.across.matrices(mats, m = num.admixtures))
...
mats <-
  llply(nms,
        .fun = function(target.pop) {
		 ...
		 lbs[src.indx] <- min.pair.pop.for.zeroing
		 ubs[src.indx] <- max.pair.pop.for.zeroing		 
                 mat <- generate.random.uniform.admixtures(tmp.pops, 1000, min.prop = 0.01, max.prop = 1,
		                                           lbs = lbs, ubs = ubs)
                 ## Randomly break the dummy prop into a src and target
		 vec <- rep(0, ncol(mat))
		 colnames(mat) <- NULL
		 mat <- rbind(mat, vec)
		 rownames(mat)[nrow(mat)] <- target.pop
...
broken.stick.admixtures.unconstrained.zero <- t(find.extremal.points.across.matrices(mats, m = num.admixtures))

## apply(broken.stick.admixtures.unconstrained.zero, 2, function(col) length(which(col==0)))

cat("Done creating broken stick admixtures\n")
save.image(".Rdata")

cat("About to zero out populations\n")

## Generate biological admixtures by:
## Launching a Markov Chain from a random seed for 10000 samples from each of those points (using hitandrun)
...
tumor.maxs <- tumor.mins + 10^-4
tumor.flat.model <- round(flat.model, digits=2)
tumor.flat.model["min", "tumor.fraction"] <- 0.55
tumor.flat.model["max", "tumor.fraction"] <- 0.80


set.seed(1234)
ndim <- ncol(tumor.flat.model)
n.tot <- 1000
##N <- 1000*ndim^3
...
N.thin <- (ndim-1)^3
admixtures <-
  llply(1:length(tumor.mins), .parallel = TRUE,
        .fun = function(i) {
	         print(tumor.mins[i])
	         model <- round(flat.model, digits=2)
		 model <- model[, !(colnames(model) == "tumor.fraction")]

                 ldply(1:n.rand, .parallel = TRUE,
                       .fun = function(j) {
                             eq.constr <- list(
                               constr = matrix(rep(1,ndim-1),nrow=1,ncol=(ndim-1)),
                               dir = '=',
                               rhs = 1-tumor.mins[i])

                             ineq.constr <- list(
                               constr = rbind(-diag(ndim-1),diag(ndim-1)),
                               dir = rep('<=',(ndim-1)*2),
			     ...
			     cat(paste0("num rows = ", nrow(df), "\n"))
                             df <- round(df, digits = 2)
			     eps <- 10^-4
    			     flag <- abs( rowSums(df) - (1 - tumor.mins[i]) ) < eps
			     df <- df[flag, ,drop=F]
			     df <- df[!duplicated(df),,drop=F]
			     vec <- rep(tumor.mins[i], nrow(df))
			     df <- cbind(vec, df)
    			     flag <- abs ( rowSums(df) - 1 ) < eps
			     df <- df[flag, ,drop=F]
			     colnames(df) <- c("tumor.fraction", colnames(model))
			     df
                      })
		    })
                 ...
                 ldply(1:n.rand, .parallel = FALSE,
                       .fun = function(j) {
                             ...
                             ineq.constr <- list(
                               ...
                               dir = rep('<=',ndim*2),
                               rhs = c(-as.numeric(tumor.flat.model["min",]),as.numeric(tumor.flat.model["max",])))
                             ...
                             x0 <- createSeedPoint(constr,homogeneous=TRUE,randomize=TRUE)
                             df <- har(x0,constr,N,transform=transform, homogeneous=TRUE)$samples

                             df <- round(df, digits = 2)
    			     flag <- rowSums(df) == 1
			     df <- df[flag, ]
			     df <- df[!duplicated(df),]
			     colnames(df) <- colnames(tumor.flat.model)
			     df
                      })
		    })
...
admixtures <- llply(admixtures, function(df) df[!duplicated(df),])

save(admixtures, file="admixtures.Rd")

cat("Done saving admixutres\n")

source("utils.R")

transposed.admixtures <- llply(admixtures, .fun = function(mat) t(mat))
extreme.admixtures <- find.extremal.points.across.matrices(transposed.admixtures, 200)

save(extreme.admixtures, file="extreme-admixtures.Rd")

cat("Done saving extreme admixtures\n")

broken.stick.admixtures.unconstrained.sv <- broken.stick.admixtures.unconstrained
broken.stick.admixtures.sv <- broken.stick.admixtures

save.image(".Rdata")
cat("Done saving image\n")


admixture.tbl <- t(extreme.admixtures)[1:num.admixtures,]
## admixture.tbl <- ldply(admixtures)
## admixture.tbl <- admixture.tbl[sample.int(nrow(admixture.tbl), size = num.admixtures, replace = FALSE),]

broken.stick.admixtures.unconstrained <- broken.stick.admixtures.unconstrained.sv[1:num.admixtures,]
broken.stick.admixtures <- broken.stick.admixtures.sv[1:num.admixtures,]
...
source("utils.R")
calc.entropy <- function(ps) { return(-1 * sum(unlist(lapply(ps, calc.p.log.p)))) }
entropies <- unlist(apply(admixture.tbl, 1, function(row) calc.entropy(row)))
sorted.entropies <- sort(entropies, decreasing=TRUE)
cutoff <- sorted.entropies[100]
flag <- entropies > cutoff
head(admixture.tbl[flag,])
stop("stop")

ndim <- ncol(flat.model)
## Create the simplex constraints
flat.model["min","tumor.fraction"] <- 0.55
flat.model["max","tumor.fraction"] <- 0.8
eps <- 10^-4
lbs <- lapply(1:ndim, function(i) lowerBoundConstraint(ndim, i, flat.model["min",i]-eps))
ubs <- lapply(1:ndim, function(i) upperBoundConstraint(ndim, i, flat.model["max",i]+eps))
userConstr <- Reduce("mergeConstraints", c(ubs, lbs))
userConstr <- mergeConstraints(lowerBoundConstraint(ndim, 1, 0.01))
userConstr <- lowerBoundConstraint(ndim, 1, 0.01)
transform <- simplex.createTransform(ndim)
constr <- simplex.createConstraints(transform, userConstr)
constr <- simplex.createConstraints(transform, userConstr=NULL)
constr <- transformConstraints(transform, userConstr)

vec <- as.numeric(admixture.tbl[1,])
seedPoint <- as.numeric(admixture.tbl[1,])
N <- 10000
...
samples <- bbReject(lb, ub, constr, 10)$samples

homogeneous <- FALSE
transform <- simplex.createTransform(ndim, inverse = homogeneous)
constr <- simplex.createConstraints(transform, userConstr=NULL)
seedPoint <- createSeedPoint(constr, homogeneous=homogeneous, randomize=TRUE)
...
ineq.constr <- list(
  ...
  dir = rep('<=',n*2),
  rhs = c(-as.numeric(flat.model["min",]),as.numeric(flat.model["max",])))
...
x0 <- createSeedPoint(constr,homogeneous=FALSE)
x0 <- as.numeric(vec %*% transform)
inv.transform <- createTransform(basis, inverse=TRUE, keepHomogeneous=FALSE)
x0 <- as.numeric(inv.transform %*% vec)
x0 <- as.numeric(vec %*% inv.transform)
x0 <- vec
x0 <- basis$basis %*% vec + basis$translate
x0 <- as.numeric(transform %*% vec)
x0 <- t(as.matrix(vec) - basis$translate) %*% transform 
x0[length(x0)] <- 1
x <- har(x0,constr,500,transform=transform, homogeneous=TRUE)$samples
constr$constr %*% x0 <= constr$rhs

x0 <- createSeedPoint(constr,homogeneous=TRUE,randomize=TRUE)
N <- 1000*n^3
x <- har(x0,constr,N,transform=transform, homogeneous=TRUE)$samples
