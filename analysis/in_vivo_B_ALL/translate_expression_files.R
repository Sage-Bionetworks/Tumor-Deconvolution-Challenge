...
counts_transcripts_tbl <- "syn22492032" %>% 
    synapse_file_to_tbl() %>% 
    dplyr::mutate(
...
synapser::File("ensg_counts.csv", parent = "syn22492020") %>% 
    synapser::synStore()
...
synapser::File("symbol_counts.csv", parent = "syn22492020") %>% 
    synapser::synStore()
file.remove("symbol_counts.csv")

tpm_transcripts_tbl <- "syn22492026" %>% 
    synapse_file_to_tbl() %>% 
    dplyr::mutate(
        transcript = stringr::str_remove_all(transcript, ".[:digit:]+$")
    ) 
...
synapser::File("ensg_tpm.csv", parent = "syn22492020") %>% 
    synapser::synStore()
...
synapser::File("symbol_tpm.csv", parent = "syn22492020") %>% 
    synapser::synStore()
