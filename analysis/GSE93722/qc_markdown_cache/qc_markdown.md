...
```r
make_cibersort_vs_ground_truth_plots <- function(
    cibersort_results, ground_truth, cibersort_common_groups, gt_cs_groups,
    cs_gt_groups){
    
    results_df <- cibersort_results %>%
        dowload_and_format_cibersort_df %>%  
        group_cell_types(cs_gt_groups) %>% 
        select(c("sample", cibersort_common_groups)) %>% 
...
make_cibersort_vs_ground_truth_plots(
    config$synapse_ids$cibersort_results,
    config$synapse_ids$ground_truth,
    config$cibersort_common_groups,
    config$gt_cs_groups,
    config$cs_gt_groups)
```
...
create_cibersort_gene_heatmaps <- function(config){
    ...
    zscore_matrix <- config$synapse_ids$log_tpm_expression %>% 
        create_df_from_synapse_id %>% 
        df_to_matrix("Hugo") %>% 
        .[rowSums(.) > 0,] %>% 
...
```
## Warning in .local(expr, gset.idx.list, ...): 2genes with constant
